...
function saveFollow(req, res) {
    ...
    follow.save((err, followStored) => {
        if (err) return res.status(500).send({ messsage: 'Error al guardar follow' });
...
function deleteFollow(req, res) {
    ...
    Follow.find({ 'user': userId, 'followed': followId }).remove(err => {
        ...
        return res.status(200).send({ message: 'Has hecho Unfollow!' });
    })
}
...
function getFollowingUsers(req, res) {
    ...
    Follow.find({ user: userId }).populate({ path: 'followed' }).paginate(page, itemsPerPage, (err, follows, total) => {
        if (err) return res.status(500).send({ message: 'Error en servidor, intentelo más tarde' });
...
function getFollowedUsers(req, res) {
    ...
    Follow.find({ followed: userId }).populate('user').paginate(page, itemsPerPage, (err, follows, total) => {
        ...
        if (!follows) return res.status(404).send({ message: 'No te esta siguiendo otro usuario en este momento' });
...
function getMyFollows(req, res) {
    var userId = req.user.sub;
    var followed = req.params.folowed;
